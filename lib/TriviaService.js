...
class TriviaService {
   ...
   */
  static loadCustomQuestions(selectedPackIds = []) {
    let customQuestions = [];
    const availablePacks = TriviaService.getAvailablePacks();

    const activePacks = (selectedPackIds && selectedPackIds.length > 0)
      ? availablePacks.filter(p => selectedPackIds.includes(p.id))
      : availablePacks;
